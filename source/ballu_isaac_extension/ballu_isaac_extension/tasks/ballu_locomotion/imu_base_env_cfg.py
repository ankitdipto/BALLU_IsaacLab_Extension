...
class BALLUSceneCfg(InteractiveSceneCfg):
    """Configuration for a BALLU scene with IMU sensor."""
    ...
    # BALLU robot with IMU sensor
    robot: ArticulationCfg = BALLU_CFG.replace(
        prim_path="{ENV_REGEX_NS}/Robot",
        # sensors={
        #     "imu": SensorAttachmentCfg(
        #         cfg=ImuCfg(
        #             prim_path="{ENV_REGEX_NS}/Robot/imu",
        #             update_period=0,  # Update every simulation step
        #             debug_vis=True,
        #         ),
        #         # Attach to the robot's base/root
        #         body_name="base",
        #         offset_pos=(0.0, 0.0, 0.0),  # Center of the base
        #         offset_rot=(1.0, 0.0, 0.0, 0.0),  # No rotation
        #     )
        # }
    )
    ...
    imu_pelvis = ImuCfg(
        ...
        gravity_bias=(0.0, 0.0, 9.81), # Compensates 'g'. In this config, at rest, IMU reads (0.0, 0.0, 0.0)
        debug_vis=True,
    )
...
class ObservationsCfg:
    """Observation specifications for the MDP with only IMU sensor data."""

    @configclass
    class PolicyCfg(ObsGroup):
        """Observations for policy group using IMU sensor data only."""
        ...
        # IMU sensor readings using built-in functions
        imu_orientation = ObsTerm(
            func=mdp.imu_orientation, 
            params={"asset_cfg": SceneEntityCfg("imu_pelvis")}
        )
...
@configclass
class BALLU_IMU_base_EnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the BALLU robot environment with IMU sensor observations."""
