...
class BALLUPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    max_iterations = 1500
    save_interval = 100
    experiment_name = "lab_02.24.2026"
...
@configclass
class BALLUSpecialistPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """Runner config for fixed 4-way GCR/SPCF specialist routing."""

    num_steps_per_env = 20
    max_iterations = 1500
    save_interval = 100
    experiment_name = "lab_02.10.2026"
    ...
    policy = RslRlPpoActorCriticCfg(
        class_name="SpecialistActorCritic",
        init_noise_std=0.5,
        actor_hidden_dims=[128, 64, 32],
        critic_hidden_dims=[128, 64, 32],
        activation="elu",
...
class BALLUMoEPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    policy = RslRlMoEActorCriticCfg(
        ...
        activation="elu",
        num_experts=5,
        num_morphology_obs=11,  # BALLU morphology vector dimension (11 + 1 cluster ID)
...
@configclass
class BALLUSoftMoEPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """Configuration for BALLU MoE (Mixture-of-Experts) PPO training.
    ...
    policy = RslRlMoEActorCriticCfg(
        ...
        # Expert diversity loss (top-2 cosine repulsion on action means)
        # Scheduled inside `MoEActorCritic.sample_and_commit_experts(...)` at the start of each PPO iteration.
        diversity_coef_max=0.0, #1.0e-3,
        diversity_start_iter=-1,
        diversity_ramp_iters=-1,
        diversity_eps=1.0e-8,
    )
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        moe_cfg=RslRlMoECfg(
            num_morphology_obs=11,
            routing_type="soft",  # Must match policy routing_type
            load_balance_coef=0.01,
            gate_probs_log_interval=3,  # Log gate probs every N iterations
            gate_probs_num_envs=4,  # Log first N environments
        ),
