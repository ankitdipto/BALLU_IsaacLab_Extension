...
- Throughout the repository, the name `ballu_isaac_extension` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of ballu_isaac_extension (in files/directories) to your_fancy_extension_name
...
We provide an example UI extension that will load upon enabling your extension defined in `source/ballu_isaac_extension/ballu_isaac_extension/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/ballu_isaac_extension"
    ]
}
```
