...
def run_training_experiment(
        morph_id: str,
        knee_stiffness: float = 0.4, 
        spring_stiffness: float = 0.0107,
        task: str = "Isc-Vel-BALLU-1-obstacle",
        max_iterations: int = 50,
        seed: int = 42,
        knee_damping: float = 0.06,
        spring_damping: float = 0.001,
        gravity_comp_ratio: float = 0.891
    ) -> tuple[bool, float]:
    ...
    run_name = f"{morph_id}_knkp{knee_stiffness:.3f}_knkd{knee_damping:.3f}_spcf{spring_stiffness:.3f}_spd{spring_damping:.3f}_gcr{gravity_comp_ratio:.3f}_seed{seed}"
    cmd = [
        ...
        f"env.scene.robot.actuators.knee_effort_actuators.pd_p={knee_stiffness}",
        f"env.scene.robot.actuators.knee_effort_actuators.pd_d={knee_damping}",
        f"env.scene.robot.actuators.knee_effort_actuators.spring_coeff={spring_stiffness}",
        f"env.scene.robot.actuators.knee_effort_actuators.spring_damping={spring_damping}"
    ]

    env = os.environ.copy()
    env['BALLU_USD_REL_PATH'] = f"morphologies/10.28.2025/{morph_id}/{morph_id}.usd"
...
def objective(
    ...
    knee_stiffness = trial.suggest_float("Kp_knee", 0.8, 1.2) # (0.8, 1.2)
    knee_damping = trial.suggest_float("Kd_knee", 0.02, 0.1) # (0.02, 0.1)
    spring_stiffness = trial.suggest_float("Kp_spring", 0.01, 0.04) # (0.01, 0.03)
    spring_damping = trial.suggest_float("Kd_spring", 0.001, 0.02) # (0.001, 0.02)
    gravity_comp_ratio = trial.suggest_float("gravity_comp_ratio", 0.82, 0.90) # (0.82, 0.90)
...
def main():
    ...
    parser.add_argument("--morph_id", type=str, default="reprod_asset_FL_0.64", help="Morphology ID (default: reprod_asset_FL_0.64)")
    ...
    try:
        study.optimize(
            lambda trial: objective(trial, MORPH_ID, MAX_ITERATIONS, SEED, TASK, DEVICE, TEST_VIDEO_LENGTH, TEST_NUM_ENVS),
            n_trials=N_TRIALS, 
    ...
    # Save summary
    summary_path = f"{project_dir}/logs/optuna/10.28.2025/{STUDY_NAME}_summary.json"
