...
def run_training_experiment(
    ...
    """Run training experiment and return success status and best reward."""
    ...
    best_reward = ckpt_dict["best_reward"]
    
    return True, best_reward, log_dir
...
def run_testing_experiment(
    ...
    test_script_path = f"{project_dir}/scripts/rsl_rl/play.py"
    cmd = [
        sys.executable,
        test_script_path,
        "--task", task,
    ...
    try:
        for line in process.stdout:
            print(line, end='')
            if "base_vel_mean:" in line:
                # Extract the value after the colon, handling any whitespace
                parts = line.split("base_vel_mean:")
                if len(parts) > 1:
                    try:
                        value_str = parts[1].strip().split()[0]  # Take first token after colon
                        base_vel_mean = float(value_str)
                    except (ValueError, IndexError) as e:
                        raise ValueError(f"Failed to parse base_vel_mean from line: {line.strip()}, extracted value: '{parts[1].strip() if len(parts) > 1 else 'N/A'}', error: {e}")
                else:
                    raise ValueError(f"Failed to parse base_vel_mean from line: {line.strip()}")
            if "base_vel_std:" in line:
                # Extract the value after the colon, handling any whitespace
                parts = line.split("base_vel_std:")
                if len(parts) > 1:
                    try:
                        value_str = parts[1].strip().split()[0]  # Take first token after colon
                        base_vel_std = float(value_str)
                    except (ValueError, IndexError) as e:
                        raise ValueError(f"Failed to parse base_vel_std from line: {line.strip()}, extracted value: '{parts[1].strip() if len(parts) > 1 else 'N/A'}', error: {e}")
                else:
                    raise ValueError(f"Failed to parse base_vel_std from line: {line.strip()}")
        process.wait(timeout=2 * 3600)
        
        # Verify that both values were successfully parsed
        if base_vel_mean is None:
            raise ValueError("base_vel_mean was not found in the output. The testing script may have failed before printing the value.")
        if base_vel_std is None:
            raise ValueError("base_vel_std was not found in the output. The testing script may have failed before printing the value.")
    except subprocess.TimeoutExpired:
    ...
    return process.returncode == 0, test_command, base_vel_mean, base_vel_std
...
def objective(
    ...
    gravity_comp_ratio = trial.suggest_float("gravity_comp_ratio", 0.70, 0.87)
    ...
    success, best_reward, log_dir = run_training_experiment(
        final_morph_id, 
        task=task,
    ...
    print(f"[Trial {trial.number}] COMPLETED - Best reward: {best_reward:.4f}")
    ...
    trial.set_user_attr("best_reward", best_reward)
    trial.set_user_attr("log_dir", log_dir)

    # Run testing for this morphology
    run_name = log_dir.split("/")[-1] if log_dir else ""
    print(f"[Trial {trial.number}] Starting testing...")
    test_success, test_command, base_vel_mean, base_vel_std = run_testing_experiment(
...
def main():
    ...
    parser.add_argument("--n_trials", type=int, default=100, help="Number of optimization trials (default: 40)")
    parser.add_argument("--max_iterations", type=int, default=1000, help="Max training iterations per trial (default: 1600)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for training (default: 42)")
    parser.add_argument("--task", type=str, default="Isc-BALLU-fast-walk", help="Task name (default: Isc-BALLU-fast-walk)")
    ...
    parser.add_argument("--test_num_envs", type=int, default=10, help="Number of envs for testing (default: 1)")
